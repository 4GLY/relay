...
func buildMux(handler Handler, cfg config.Config, runtime app.Runtime) *http.ServeMux {
	...
	mux.HandleFunc("/v1/settings/api-keys", requireUserSessionCookie(handler.services, handler.handleUserAPIKeys))
	mux.HandleFunc("/v1/settings/api-keys/revoke", requireUserSessionCookie(handler.services, handler.handleRevokeUserAPIKey))
	mux.HandleFunc("/v1/settings/provider-credentials", requireSessionOrAdmin(adminToken, handler.services, handler.handleProviderCredentials))
	mux.HandleFunc("/v1/settings/provider-credentials/", requireSessionOrAdmin(adminToken, handler.services, handler.handleProviderCredential))
